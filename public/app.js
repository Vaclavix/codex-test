...
const search = async () => {
  ...
  try {
    ...
    if (data.warning) {
      setStatus(`${data.warning} Vyber firmu ze seznamu.`, true);
    } else {
...
const renderValuation = ({ company, sourceData, dcf, source, warning }) => {
  ...
  const sourceLabel = source === 'realtime' ? 'Realtime data z webu' : 'Offline fallback data';
